class ContestRequestsController < ApplicationController
   ...
   def save_lookbook
     #raise session[:lookbook].inspect
     lookbook = Lookbook.find_by_designer_id_and_contest_id(session[:designer_id], params[:id])
     
     if lookbook.blank?
        lookbook = Lookbook.new({:designer_id => session[:designer_id], :contest_id => params[:id], :feedback =>  session[:lookbook]['feedback']})
        if lookbook.save()
            lid = lookbook.id
        else
     ...
     if lid.present?
       LookbookDetail.destroy_all(:lookbook_id => lid)
       if session[:lookbook]['lookbook_photo']['document_id'].present?
         ...
         document_ids.each_with_index do |doc, index|
           LookbookDetail.create({:lookbook_id => lid, :document_id => doc, :doc_type => 1, :description => document_titles[index]})    
         end
       end
       if session[:lookbook]['lookbook_link']['links'].present?
         ...
         document_ids.each_with_index do |doc, index|
           if doc.present?
             LookbookDetail.create({:lookbook_id => lid, :url => doc, :doc_type => 2, :description => document_titles[index]})    
           end 
         end
       end
